...
-- 7. country에서 독립 년도에 대한 기록이 없고 
-- 인구가 10000이상인 국가의 정보를 인구의 오름차순으로 출력하시오.(29건)
select code, name, gnp, Population
from country
where IndepYear is null and population>=10000
order by population;

-- 8. country에서 인구가 1억<=x<=2억 인 나라를 
-- 인구 기준으로 내림차순 정렬해서 상위 3개만 출력하시오.
...
-- 단 독립 년이 같다면 code를 기준으로 내림차순 한다.(7건)
...
-- name의 길이가 10 이상인 자료를 name 길이의 내림차순으로 상위 10개만 출력하시오. (10건) 
...
-- 13. country에서 code가 모음으로 시작하는 나라의 정보를 출력하시오.
-- 이때 name의 오름차순으로 정렬하고 3번 부터 3개만 출력한다.
select code, name
from country
...
select name, replace(name, substring(name,3,length(name)-4),REPEAT("*",(length(name)-4))) guess
from country;
...
-- 16. country에서 인구가 1억 이상인 국가들의 1인당 점유면적(surfacearea/population)을 
-- 반올림해서 소숫점 3자리로 표현하시오. 1인당 점유 면적의 오름차순으로 정렬한다.(10건)
...
select curdate() 오늘, dayofyear(now()) "올해 지난 날", date_format(date_add(now(), Interval 100 day), '%Y-%m-%d') "100일 후";

-- 18. country에서 asia에 있는 나라 중 희망 수명이 있는 경우에 
...
select name, gnp, gnpold, if(ISnull(gnpold), '신규', gnp-gnpold) as "gnp 향상"
from country
order by name;

-- 20. 2020년 어린이 날이 평일이면 행복, 토요일 또는 일요일이면 불행이라고 출력하시오.
select weekday('2020-05-05'), if(weekday('2020-05-05')<=4, '행복', '불행') as "행복여부";